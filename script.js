/**
 * script.js - キン乱シミュレーター CSP/武将選択/2枠宿縁 完全対応版
 */
...
let belongsCategories = []; // bushos-data.jsから抽出される所属リスト
...
function initApp() {
    // 1. bushosDataから所属(belongs)をユニーク抽出
    if (window.bushosData) {
        const belongsSet = new Set();
        window.bushosData.forEach(b => {
            if (Array.isArray(b.belongs)) {
                b.belongs.forEach(cat => belongsSet.add(cat));
            }
        });
...
function renderCategoryList(type) {
    ...
    items.forEach(it => {
        ...
        btn.innerText = it;
        btn.addEventListener('click', () => renderBushoList(type, it));
...
function renderBushoList(type, val) {
    ...
    back.addEventListener('click', () => renderCategoryList(type));
    ...
    filtered.forEach(b => {
        ...
        btn.addEventListener('click', () => selectBusho(b));
        body.appendChild(btn);
    });
}
